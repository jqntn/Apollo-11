...
static int
menu_read_key(int* selected_index)
{
  int ch;
  int next;
#ifndef _WIN32
...
static void
menu_render(const backend_option_t* options, int count, int selected)
{
  int i;
  ...
  if (first_render) {
    ...
    sprintf(
      line_buffer, "Select display mode (Up/Down + Enter, or 1-%d):", count);
    term_set_cursor(7, 0);
