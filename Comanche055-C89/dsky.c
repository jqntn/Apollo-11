...
dsky_init(void)
{
  ...
  dsky_display.prog[0] = -1;
  dsky_display.prog[1] = -1;
...
static void
light_str(char* buf, int on, const char* name)
{
  if (on) {
    int i, len;
    buf[0] = '[';
...
dsky_poll_input(void)
{
  ...
  switch (ch) {
    ...
    case 'v':
    case 'V':
      keycode = DSKY_KEY_VERB;
      break;
    case 'n':
    case 'N':
      keycode = DSKY_KEY_NOUN;
      break;
    case '+':
    case '=':
      keycode = DSKY_KEY_PLUS;
      break;
    case '-':
    case '_':
      keycode = DSKY_KEY_MINUS;
      break;
    case 'e':
    case 'E':
    case '\r':
    case '\n':
      keycode = DSKY_KEY_ENTR;
      break;
    case 'c':
    case 'C':
      keycode = DSKY_KEY_CLR;
      break;
    case 'p':
    case 'P':
      keycode = DSKY_KEY_PRO;
      break;
    case 'k':
    case 'K':
      keycode = DSKY_KEY_KREL;
      break;
    case 'r':
    case 'R':
      keycode = DSKY_KEY_RSET;
      break;
    case 'q':
    case 'Q':
      term_cleanup();
...
static void
console_be_cleanup(void)
{
  term_cleanup();
  hal_term_cleanup();
}

dsky_backend_t dsky_console_backend = { console_be_init,
                                        dsky_update,
                                        dsky_poll_input,
                                        console_be_cleanup,
                                        hal_sleep_ms };
