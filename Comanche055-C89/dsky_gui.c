...
static const unsigned char seg_tab[10] = { SA | SB | SC | SD | SE | SF,
                                           SB | SC,
                                           SA | SB | SD | SE | SG,
                                           SA | SB | SC | SD | SG,
                                           SB | SC | SF | SG,
...
static void
frect(HDC dc, int x, int y, int w, int h, COLORREF c)
{
  RECT r;
  HBRUSH br;
  r.left = x;
...
static void
draw_seg(HDC dc, int x, int y, int digit)
{
  unsigned char s;
  ...
  frect(
    dc, x + g_seg_w - g_seg_t, y + g_seg_t, g_seg_t, hh, (s & SB) ? on : off);
  frect(dc,
        x + g_seg_w - g_seg_t,
        y + g_seg_t + hh + g_seg_t,
        g_seg_t,
        hh,
        (s & SC) ? on : off);
  frect(
    dc, x + g_seg_t, y + g_seg_h - g_seg_t, hw, g_seg_t, (s & SD) ? on : off);
...
static void
draw_sign(HDC dc, int x, int y, int sign)
{
  COLORREF on = COL_SEG_ON, off = COL_SEG_OFF;
  int cx = x + g_seg_w / 2, cy = y + g_seg_h / 2;
  frect(
    dc, cx - g_seg_w / 2, cy - g_seg_t / 2, g_seg_w, g_seg_t, sign ? on : off);
  frect(dc,
        cx - g_seg_t / 2,
        cy - g_seg_w / 2,
        g_seg_t,
...
static void
draw_light(HDC dc, int x, int y, int w, int h, int on, const char* lbl)
{
...
static void
draw_label(HDC dc, int x, int y, const char* txt)
{
...
static void
draw_btn(HDC dc, const gbtn_t* b, int pressed)
{
  RECT r;
  HFONT old;
  HBRUSH br;
...
paint_dc(HDC dc)
{
  ...
  draw_light(
    dc, S(260), S(12), S(108), S(26), dsky_display.light_prog_alarm, "PROG");
  draw_light(dc,
             S(20),
             S(44),
             S(108),
  ...
  draw_light(
    dc, S(260), S(44), S(108), S(26), dsky_display.light_restart, "RESTART");
  draw_light(
    dc, S(20), S(76), S(108), S(26), dsky_display.light_no_att, "NO ATT");
  draw_light(
    dc, S(140), S(76), S(108), S(26), dsky_display.light_key_rel, "KEY REL");
  draw_light(
    dc, S(260), S(76), S(108), S(26), dsky_display.light_tracker, "TRACKER");
  draw_light(
    dc, S(20), S(108), S(108), S(26), dsky_display.light_opr_err, "OPR ERR");
  ...
  /* COMP ACTY */
  draw_light(
    dc, S(22), S(155), S(88), S(22), dsky_display.light_comp_acty, "COMP ACTY");
...
map_char(int ch)
{
  switch (ch) {
    ...
    case 'k':
    case 'K':
      return DSKY_KEY_KREL;
    case 'r':
    case 'R':
      return DSKY_KEY_RSET;
  }
  return -1;
}

static LRESULT CALLBACK
wndproc(HWND hw, UINT msg, WPARAM wp, LPARAM lp)
{
  switch (msg) {
    case WM_PAINT: {
      PAINTSTRUCT ps;
...
{
  if (g_fnt_light) {
    DeleteObject(g_fnt_light);
    g_fnt_light = NULL;
  }
  if (g_fnt_label) {
    DeleteObject(g_fnt_label);
    g_fnt_label = NULL;
  }
  if (g_fnt_btn) {
    DeleteObject(g_fnt_btn);
    g_fnt_btn = NULL;
  }
  if (g_hwnd) {
...
dsky_backend_t dsky_gui_backend = { gui_init,
                                    gui_update,
                                    gui_poll,
                                    gui_cleanup,
                                    gui_sleep };
...
dsky_backend_t dsky_gui_backend = { stub_init,
                                    stub_update,
                                    stub_poll,
                                    stub_cleanup,
                                    stub_sleep };
